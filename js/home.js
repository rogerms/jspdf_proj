...
function process()
{
	...
	var doc = new jsPDF({format: 'legal'});
	...
	var inpos = y -2;
	doc.text(9, inpos, val('jobname'));	
	doc.text(166, inpos, val('date'));
	inpos += 20;
	doc.text(9, inpos, val('jobaddress'));
	inpos += 10;
	doc.text(9, inpos, val('home'));
	inpos += 11;
	doc.text(9, inpos, val('contact')); 
	doc.setFontSize(9);
	...
	//doc.setFontSize(11); 
	//add logo
	doc.addImage(getBase64Image('logo_srp.jpg'), 7, 10, 50, 25);
